...
# MAGIC  - Use a popular open source embedding model to embed your dataset into <a href="https://docs.databricks.com/en/generative-ai/vector-search.html">Databricks Vector Search</a>
...
# endpoints = {"vs_endpoint_ccims_fallback", "vs_endpoint_synthetic_claims_data"}
...
# "OEM_CODE":1608,"DEALER":"22887","DISTR":"1694","FC":"3382.0,3446.0,5177.0,1939.0","ENGINE_NAME_DESC":"X15 1 2017","FAILCODE":"THAC","SHOPORDERNUM":"3037900","FAILURE_MODE_BUCKET":"ACT(E) ELECTROLYTIC C26 FAILURE",

# "OEM_CODE":1457,"DEALER":"24989","DISTR":"1693","FC":"687.0,4616.0","ENGINE_NAME_DESC":"X15 3 2017","FAILCODE":"ELTS","SHOPORDERNUM":"1032900","FAILURE_MODE_BUCKET":"ACT(E) ELECTROLYTIC C26 FAILURE",
...
# def process_fc_column(df):
#     # Convert the FC column from comma-separated strings to lists of floats
#     df['FC'] = df['FC'].apply(lambda x: list(map(float, x.split(','))) if isinstance(x, str) else x)
#     return df
...
# # Helper function to clean and process the 'FC' column
# def process_fc_column(df):
#     # Convert the FC column from comma-separated strings to lists of floats
#     df['FC'] = df['FC'].apply(lambda x: list(map(float, x.split(','))) if isinstance(x, str) else x)
#     return df
...
# eval_predictions = pd.DataFrame(eval_df['predictions'].values.tolist(), columns=['predicted_failure_mode'])

# synthetic_inputs = pd.DataFrame(synthetic_df['inputs'].values.tolist())
# synthetic_predictions = pd.DataFrame(synthetic_df['predictions'].values.tolist(), columns=['predicted_failure_mode'])

# # Process the 'FC' column in both DataFrames
# eval_inputs = process_fc_column(eval_inputs)
# synthetic_inputs = process_fc_column(synthetic_inputs)

# # Combine inputs with predictions
# eval_df_full = pd.concat([eval_inputs, eval_predictions])
# synthetic_df_full = pd.concat([synthetic_inputs, synthetic_predictions])
...
#             model=None,  # assuming you're comparing data, not model
#             data=eval_df_full,
#             targets='FAILURE_MODE_BUCKET',
#             predictions='predicted_failure_mode',
...
# Helper function to clean and process the 'FC' column and convert other numeric columns
# def process_columns(df):
#     # Convert the FC column from comma-separated strings to lists of floats
#     df['FC'] = df['FC'].apply(lambda x: list(map(float, x.split(','))) if isinstance(x, str) else x)
    
#     # Ensure OEM_CODE is an integer, or you can convert it to float if needed
#     df['OEM_CODE'] = pd.to_numeric(df['OEM_CODE'], errors='coerce')
    
#     # Convert other numeric columns to float if necessary
#     # Assuming SHOPORDERNUM and others might be numeric, modify as needed
#     df['SHOPORDERNUM'] = pd.to_numeric(df['SHOPORDERNUM'], errors='coerce')
...
# Process the columns in both DataFrames
# eval_inputs = process_columns(eval_inputs)
# synthetic_inputs = process_columns(synthetic_inputs)
...
# Print out the types to check for any issues
print("Eval DF types:\n", eval_df_full.dtypes)
print("Synthetic DF types:\n", synthetic_df_full.dtypes)
...
with mlflow.start_run() as run:
    ...
    real_eval = mlflow.evaluate(
        data=eval_df,
        model_type="question-answering",
        predictions="predictions",
        extra_metrics=[question_quality],  # Include custom metric for real data
    )
    
    # Evaluate the synthetic data
    synthetic_eval = mlflow.evaluate(
        data=synthetic_df,
        model_type="question-answering",
        predictions="predictions",
        extra_metrics=[question_quality],  # Include custom metric for synthetic data
    )
    
# Compare the results from real_eval and synthetic_eval
print(f"Real Data Evaluation: {real_eval}")
print(f"Synthetic Data Evaluation: {synthetic_eval}")
