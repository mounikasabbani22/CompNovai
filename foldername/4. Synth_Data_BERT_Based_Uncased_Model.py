...
# MAGIC   <img src="/Volumes/cmidev/default/preventech/DBX_hack/compnovai_logo.PNG" alt="Image">  
# MAGIC </div>
...
# MAGIC %md
# MAGIC
# MAGIC We will load an open-sourced embedding model. Then, we will fine-tune this embedding model based on the training data.
...
# Assume that logits is a PyTorch tensor of shape (batch_size, num_classes)  

def get_probability(logits):
# Apply softmax to the logits to get the probability distribution over the classes  
  probs = F.softmax(logits, dim=1) 
  max_probs, predicted_classes = torch.max(probs, dim=1)
  return max_probs, predicted_classes
...
# MAGIC %md
# MAGIC Inference the model with test data
...
result_df['document'] = test_claims_list
result_df['actual_label'] = actual_label
result_df['prediction'] = predicted_label_fh
result_df['probability'] = probability_fh
display(result_df)
